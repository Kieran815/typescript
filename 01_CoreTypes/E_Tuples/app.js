...
person1.role[1] = 10;
console.log("Array of `person.role` (not at tuple, since it can push 'author' and change `person.role[1]` to 10 on `person.role`):");
console.table(person1.role);
...
console.log("Tuples are part of TypeScript in dev terminal, but JavaScript still reads them as normal arrays.");
console.log("NOTE: `push` still works, since `person.role` is still an array in JS, not a tuple");
console.table(person2.role);
console.log("Tuples add strictness to the type of data you are working with in development. Although the array can still mutate based on the JS, TypeScript will allow you to be more exact about specific data types in the dev terminal, ensuring higher accuracy in execution.");
